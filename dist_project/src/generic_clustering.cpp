// https://pcl.readthedocs.io/projects/tutorials/en/master/conditional_euclidean_clustering.html
...
main()
{
    ...
    std::cerr << "Loading...\n", tt.tic();
    pcl::io::loadPCDFile("/home/computer/dense_cloud.pcd", *cloud_in);
    ...
    ne.setRadiusSearch(1.0); // radius in meters for the estimation of the normals (for scale the whole map is around 10 meters)
    ...
    cec.setClusterTolerance(.4); // radius for the k-NN searching, used to find the candidate points.
    cec.setMinClusterSize(cloud_with_normals->size() / 1000);// 1% of the cloud’s total points are considered too small.
    cec.setMaxClusterSize(cloud_with_normals->size() / 1);   // 100% of the cloud’s total points are considered too Big of a cluster.
    ...
    std::cerr << "Saving...\n", tt.tic();
    pcl::io::savePCDFile("/home/computer/clustered.pcd", *cloud_out);
    std::cerr << ">> Done: " << tt.toc() << " ms\n";
    return (0);
}